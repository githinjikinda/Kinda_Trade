                ...
                <div class="col-md-6">
                    <div class="card">
                        ...
                        <div class="card-body">
                            ...
                            @endif
                            <div class="px-3">
                                <form class="form" method="post" action="{{ route('admin.attribute.update') }}" enctype="multipart/form-data">
                                @csrf
                                    <div class="form-body">
                                        <input type="hidden" name="attribute_id" id="attribute_id" value="{{$data->id}}" class="form-control">
                                        <div class="form-group">
                                            <label for="attribute">{{ trans('labels.attribute') }}</label>
                                            <input type="text" class="form-control" name="attribute" id="attribute" value="{{$data->attribute}}" placeholder="{{ trans('placeholder.attribute') }}">
                                            @error('attribute')<span class="text-danger">{{ $message }}</span>@enderror
                                        </div>
                                    </div>
                                    <div class="form-actions center">
                                        <a href="{{ route('admin.attribute') }}" class="btn btn-raised btn-warning mr-1"><i class="ft-x"></i> {{ trans('labels.cancel') }}</a>
